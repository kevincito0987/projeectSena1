...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function moveBarToIcon(iconElement) {
        ...
        const barWidth = 20; // Ancho fijo de 20px
        const barPosition = iconRect.left - footerRect.left + (iconRect.width / 2) - (barWidth / 2);

        movingBar.style.transform = `translateX(${barPosition}px)`;
        movingBar.style.width = `${barWidth}px`; // Establecer el ancho fijo de la barra
    }
